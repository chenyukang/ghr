...
- Details pane with rendered Markdown, clickable links, inline image previews with native terminal protocols when available and clickable attachment fallbacks, fenced code blocks with lightweight Rust and plain/log highlighting, descriptions, comments, review comments, PR commit activity, labels, milestones, action hints, and check summaries.
...
```bash
curl -fsSL https://raw.githubusercontent.com/chenyukang/ghr/main/install.sh | GHR_VERSION=v0.7.7 GHR_INSTALL_DIR=/usr/local/bin sh
```
...
Image previews use native terminal protocols on known compatible terminals, fall back to terminal-safe half-block rendering elsewhere, and cache downloaded image bytes under `~/.ghr/image-cache`. Set `GHR_IMAGE_PREVIEW_PROTOCOL=iterm2`, `kitty`, `sixel`, or `halfblocks` to force a renderer; use `GHR_IMAGE_PREVIEW_PROTOCOL=off` to disable inline previews.
