...
use crate::github::{
    MergeMethod, PullRequestReviewCommentTarget, approve_pull_request, close_pull_request,
...
fn draw_pr_action_dialog(
    ...
    let dialog_height = if dialog.action == PrAction::Merge {
        14
    } else {
        12
    };
...
impl AppState {
    ...
    fn handle_pr_action_dialog_key(
        ...
        self.handle_pr_action_dialog_key_with_submit(key, |item, action, merge_method| {
            start_pr_action(
                item,
                action,
                merge_method,
    ...
    where
        F: FnMut(WorkItem, PrAction, Option<MergeMethod>),
    {
        ...
        F: FnMut(WorkItem, PrAction, Option<MergeMethod>),
    {
        ...
        submit(item, action, merge_method);
    }
...
"#,
    ...
    fn merge_confirmation_switches_methods() {
        ...
        app.handle_pr_action_dialog_key_with_submit(key(KeyCode::Char('s')), |_, _, _| {
        ...
        app.handle_pr_action_dialog_key_with_submit(key(KeyCode::Char('r')), |_, _, _| {
            panic!("method switch should not submit");
        });
    ...
    fn merge_confirmation_cycles_methods_with_tab() {
        ...
        app.handle_pr_action_dialog_key_with_submit(key(KeyCode::Tab), |_, _, _| {
            panic!("method switch should not submit");
        });
        assert_eq!(
            app.pr_action_dialog
